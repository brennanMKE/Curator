#!/usr/bin/env zsh
# Read-only checks that this Mac and this commit are ready to release Curator.
# Modelled on Batty's scripts/preflight.sh. Changes nothing; the notary check makes one
...
PBXPROJ="$REPO_ROOT/Curator.xcodeproj/project.pbxproj"
...
section "Tools"
for tool in xcodebuild codesign hdiutil ditto spctl git gh openssl; do
...
if [[ "$VERSION" =~ '^[0-9]+\.[0-9]+\.[0-9]+$' ]]; then
    pass "MARKETING_VERSION $VERSION (Config/App.xcconfig)"
else
    fail "MARKETING_VERSION '$VERSION' in Config/App.xcconfig isn't X.Y.Z"
fi
if grep -qE "MARKETING_VERSION|CURRENT_PROJECT_VERSION" "$PBXPROJ"; then
    fail "the project file sets a version; remove it so Config/App.xcconfig is the only source"
else
    pass "project file doesn't override the version"
fi
...
if git -C "$REPO_ROOT" rev-parse -q --verify "refs/tags/$TAG" >/dev/null; then
    if [[ "$(git -C "$REPO_ROOT" rev-list -n1 "$TAG")" == "$(git -C "$REPO_ROOT" rev-parse HEAD)" ]]; then
        ...
        fail "tag $TAG exists on another commit; bump MARKETING_VERSION"
    fi
else
